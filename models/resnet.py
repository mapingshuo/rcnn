...
def add_ResNet50_conv4_body(body_input):
    ...
    pool1 = fluid.layers.pool2d(
        ...
        pool_stride=2,
        pool_padding=1)
    res2, ckpt = layer_warp(block_func, pool1, 64, stages[0], 1, name="res2")
    checkpoints += ckpt
    if cfg.TRAIN.freeze_at == 2:
        res2.stop_gradient = True
    res3, ckpt = layer_warp(block_func, res2, 128, stages[1], 2, name="res3")
    checkpoints += ckpt
    if cfg.TRAIN.freeze_at == 3:
        res3.stop_gradient = True
    res4, ckpt = layer_warp(block_func, res3, 256, stages[2], 2, name="res4")
    checkpoints += ckpt
    if cfg.TRAIN.freeze_at == 4:
        res4.stop_gradient = True
    return res4, checkpoints
...
def add_ResNet_roi_conv5_head(head_input, rois):
    ...
    res5, _ = layer_warp(bottleneck, pool, 512, 3, 2, name="res5")
    return res5
